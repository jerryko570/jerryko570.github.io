...
brand_colors.py (v4)
--------------------
키워드별 브랜드 컬러 매핑 + 카테고리별 대표 색(fallback).

v4 변경 (2026-09-14):
- 카테고리 팔레트를 카테고리당 1색으로 단순화 (제목 해시 다양화 제거).
  → 브랜드 매칭 안 되는 글도 카테고리 안에서 색이 흔들리지 않음.

v2 (2026-04-30):
- mcp/skills를 Claude 색에서 분리, shadcn/figjam/v0 등 추가.

전략:
1. 글의 tags/title에 브랜드 키워드 매칭 → 그 브랜드 공식 색
2. 매칭 없음 → 카테고리 대표 색 (카테고리당 고정 1색)
...
# ───────────────────────────────────────────────
# 카테고리별 대표 색 — 브랜드 매칭 안 될 때 사용
# ───────────────────────────────────────────────
# v4 (2026-09-14): 카테고리당 1색으로 단순화.
# 이전(v3)엔 카테고리마다 4-5색을 제목 해시로 골라서 같은 카테고리 글도
# 색이 제각각이었음 → 카테고리 안에서 색이 흔들리지 않도록 고정색으로 정리.
CATEGORY_COLOR: dict[str, str] = {
    "Design": "#a855f7",       # 보라
    "DesignCraft": "#f43f5e",  # 로즈
    "Frontend": "#3b82f6",     # 블루
    "DevTools": "#10b981",     # 에메랄드
    "Study": "#64748b",        # 슬레이트 (일반 스터디)
}
...
def _palette_color(category: str, title: str) -> str | None:
    ...
    v4: 제목과 무관하게 카테고리당 항상 같은 색.
    (title 인자는 pick_color 호출부 호환을 위해 유지)
    """
    return CATEGORY_COLOR.get(category)
