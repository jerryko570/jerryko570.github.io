...
공부한 내용 기준으로는, 코루틴과 `async/await`, `data class`와 타입 유틸리티처럼 비슷한 문제를 다른 언어가 어떻게 풀었는지 비교해보면 이해가 달라질 수 있겠다는 생각이 들었다. 지금 단계에서 Kotlin 문서까지 볼 여유는 없지만, 언어 설계를 비교하는 방식 자체는 흥미로운 공부 방식일 것 같다.
